...
tr p d (PRSApp prim [e1, e2]) = R.SPR prim arg1 arg2
  where
    arg1 = prsArg(tr p d e1)
    arg2 = prsArg(tr p d e2)

prsArg (R.ARG _ i) = R.Idx i
prsArg (R.INT i) = R.Lit i
prsArg other = error ("Nested PRSApp:")
...
vars :: Exp -> [Id]
vars (Var v) = [v]
vars (PRSApp p es) = concatMap vars es
vars e = []
