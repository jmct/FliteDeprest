/* =================================== */
/* REDUCERON EMULATOR WITH STATIC      */
/* SPECULATIVE EVALUATION OF           */
/* PRIMITIVE REDEXES                   */
/* Matthew N                           */
/* 23 September 2009                   */
/* =================================== */
...
typedef enum { NUM, ARG, REG, VAR, CON, FUN, PRI, SPR} AtomTag;
...
typedef enum {LITERAL, INDEX} ArgKind;
...
typedef struct { Prim id; Bool swap; PRSArg arg1; PRSArg arg2; } Spr;
...
/* Resolve PRS argument */
void resolveArg(PRSArg arg, Int argPtr, Atom *result)
{
  if (arg.kind == LITERAL) {
    result->tag = NUM;
    result->contents.num = arg.content;
  }
  else if (arg.kind == INDEX) {
    *result = stack[argPtr-arg.content];
  }
}
...
Atom inst(Int base, Int argPtr, Atom a)
{
  if (a.tag == VAR) {
    ...
    Atom x, y;
    resolveArg(a.contents.spr.arg1, argPtr, &x);
    resolveArg(a.contents.spr.arg2, argPtr, &y);
    if (a.contents.spr.swap) {
      a = prim(a.contents.spr.id, y, x);
    }
    else {
      a = prim(a.contents.spr.id, x, y);
    }
  }
  return a;
}
...
Bool parseAtom(Atom* result)
{
  ...
  return (
    ...
    ||
    (  scanf(" SPR \"%10[^\"]\" (%3s%*[ (]%i%*[) ](%3s%*[ (]%i%*[) ])",
             str, str2, &x, str3, &y)
    && perform(result->tag = SPR)
    && perform(strToPrim(str, &result->contents.spr.id,
                              &result->contents.spr.swap))
    && perform(result->contents.spr.arg1.kind =
         (!strcmp(str2, "Lit")) ? LITERAL : INDEX)
    && perform(result->contents.spr.arg2.kind =
         (!strcmp(str3, "Lit")) ? LITERAL : INDEX)
    && perform(result->contents.spr.arg1.content = x)
    && perform(result->contents.spr.arg2.content = y)
    ) 
