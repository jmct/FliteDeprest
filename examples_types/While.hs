...
data Val      = Add Val Val | N Int | Sub Val Val | V Int ;
data BVal     = And BVal BVal | Eq Val Val | FALSE | Le Val Val | Neg BVal | TRUE;
data Stm      = Ass Int Val | Comp Stm Stm | If BVal Stm Stm | Skip | While BVal Stm;
data State s  = Final s | Inter Stm s;
data List a   = Cons a (List a) | Nil; 
data Bool     = False | True;
data Pair a b = Pair a b; 


value (Cons (Pair x y) s) v k = case (==) x v of { True -> k y ; False -> value s v k };
